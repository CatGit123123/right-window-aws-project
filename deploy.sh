...
EOF
    ...
    if [ "$DEPLOYMENT_TYPE" == "pdf2pdf" ]; then
        ...
        SOURCE_VERSION="Leverg-Security"  # Use pdf2html-subtree since buildspec only exists there
        BUILDSPEC_FILE="buildspec-unified.yml"
    else
        BUILD_IMAGE="aws/codebuild/amazonlinux2-x86_64-standard:5.0"
        COMPUTE_TYPE="BUILD_GENERAL1_LARGE"
        PRIVILEGED_MODE="true"
        SOURCE_VERSION="Leverg-Security"
        BUILDSPEC_FILE="buildspec-unified.yml"
    fi
...
GITHUB_URL="https://github.com/CatGit123123/right-window-aws-project"
